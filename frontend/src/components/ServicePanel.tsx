...
export function ServicePanel({ serviceId, onClose, allowFinance = true, initialTab = "detail" }: { serviceId: string; onClose: () => void; allowFinance?: boolean; initialTab?: "detail" | "history" | "billing" }) {
  ...
  return <Modal title="Detalle del servicio" onClose={onClose} wide>
    ...
    {service && <>
      ...
      {tab === "billing" && <div className="billing-panel"><section><p className="eyebrow">SALDO DEL SERVICIO</p><MoneyValue as="strong" className="money-total" value={service.amount_due == null ? "Sin definir" : currency(service.balance)} /><div className="money-breakdown"><span>Total <MoneyValue as="b" value={service.amount_due == null ? "Sin definir" : currency(service.amount_due)} /></span><span>Cobrado <MoneyValue as="b" value={currency(service.paid_amount)} /></span></div></section><section><h4>Registrar pago</h4><PaymentForm amountDue={service.amount_due} balance={service.balance} onEditTotal={() => { setTab("detail"); setEditing(true); setError(""); }} onSubmit={submitPayment} disabled={payment.isPending} /></section><section className="payment-list"><h4>Movimientos</h4>{service.payments?.map((item) => <div key={item.id} className={item.voided_at ? "voided" : ""}><Banknote /><span><strong>{item.method_name}</strong><small>{dateTime(item.paid_at)} · {item.recorded_by_name}</small></span><MoneyValue as="b" value={currency(item.amount)} />{!item.voided_at && <button className="icon-button" title="Anular pago" onClick={() => { const reason = prompt("Motivo de la anulación:"); if (reason?.trim()) voidPayment.mutate({ id: item.id, reason }); }}><RotateCcw /></button>}</div>)}</section></div>}
    </>}
  </Modal>;
}
