...
export function ServicePanel({ serviceId, onClose, allowFinance = false, allowPaymentManagement = false, initialTab = "detail" }: { serviceId: string; onClose: () => void; allowFinance?: boolean; allowPaymentManagement?: boolean; initialTab?: "detail" | "history" | "billing" }) {
  ...
  return <Modal title="Detalle del servicio" onClose={onClose} wide>
    ...
    {service && <>
      ...
      {tab === "billing" && <div className="billing-panel"><section><p className="eyebrow">SALDO DEL SERVICIO</p><MoneyValue as="strong" className="money-total" value={service.amount_due == null ? "Sin definir" : currency(service.balance)} /><div className="money-breakdown"><span>Total <MoneyValue as="b" value={service.amount_due == null ? "Sin definir" : currency(service.amount_due)} /></span><span>Cobrado <MoneyValue as="b" value={currency(service.paid_amount)} /></span></div></section><section><h4>{allowPaymentManagement ? "Registrar pago" : "Cobranza"}</h4>{allowPaymentManagement ? <PaymentForm amountDue={service.amount_due} balance={service.balance} onEditTotal={() => { setTab("detail"); setEditing(true); setError(""); }} onSubmit={submitPayment} disabled={payment.isPending} /> : <div className="inline-alert warning">Tenés permiso para consultar importes, pero no para registrar cobros.</div>}</section><section className="payment-list"><h4>Movimientos</h4>{service.payments?.map((item) => <div key={item.id} className={item.voided_at ? "voided" : ""}><Banknote /><span><strong>{item.method_name}</strong><small>{dateTime(item.paid_at)} · {item.recorded_by_name}</small></span><MoneyValue as="b" value={currency(item.amount)} />{allowPaymentManagement && !item.voided_at && <button className="icon-button" title="Anular pago" onClick={() => { const reason = prompt("Motivo de la anulación:"); if (reason?.trim()) voidPayment.mutate({ id: item.id, reason }); }}><RotateCcw /></button>}</div>)}</section></div>}
    </>}
  </Modal>;
}
...
export function PaymentForm({ amountDue, balance, onEditTotal, onSubmit, disabled }: { amountDue?: string | null; balance?: string | null; onEditTotal: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; disabled: boolean }) {
  if (amountDue == null || Number(amountDue) <= 0) return <div><div className="inline-alert warning">Ingresá el importe total del servicio antes de registrar un pago.</div><button type="button" className="button secondary" onClick={onEditTotal}>Cargar importe total</button></div>;
  if (Number(balance ?? 0) <= 0) return <div className="inline-alert success">El servicio no tiene saldo pendiente.</div>;
  return <PaymentFields balance={balance} onSubmit={onSubmit} disabled={disabled} />;
}

function PaymentFields({ balance, onSubmit, disabled }: { balance?: string | null; onSubmit: (event: FormEvent<HTMLFormElement>) => void; disabled: boolean }) {
  ...
  return <form className="payment-form" onSubmit={onSubmit}><label>Importe<input type="number" name="amount" min="0.01" max={balance ?? undefined} step="0.01" required /></label><label>Medio<select name="method" required><option value="">Seleccionar</option>{methods.data?.map((method) => <option key={method.id} value={method.id}>{method.name}</option>)}</select></label><label className="span-2">Nota<input name="note" /></label><button className="button primary span-2" disabled={disabled}><CheckCircle2 /> Registrar cobro</button></form>;
}
