...
export function CashPage() {
  ...
  return <div className="page">
    ...
    <section className="cash-summary"><div><span>Total neto</span><MoneyValue as="strong" value={currency(report.data?.total)} /><small>{report.data?.payments.length ?? 0} movimientos relacionados</small></div><div><span>Anulado en el día</span><MoneyValue as="strong" value={currency(report.data?.voided_total)} /><small>Conservado para auditoría</small></div><div className="cash-methods"><span>Distribución neta por medio</span>{report.data?.by_method.map((item) => <p key={item.method__name}><b>{item.method__name}</b><span>{item.movements} mov.</span><MoneyValue as="strong" value={currency(item.total)} /></p>)}</div></section>
    <section className="flat-section"><header><div><p className="eyebrow">MOVIMIENTOS</p><h2>Detalle del día</h2></div></header><div className="payment-table"><div className="payment-row table-head"><span>Hora</span><span>Servicio</span><span>Medio</span><span>Usuario</span><span>Importe</span></div>{report.data?.payments.map((payment) => <div key={payment.id} className={`payment-row ${payment.voided_at ? "voided" : ""}`}><span>{dateTime(payment.voided_at || payment.paid_at)}</span><span><ReceiptText /><b>Orden {payment.service.slice(0, 8)}</b><small>{payment.note || "Sin nota"}</small></span><span><WalletCards /> {payment.method_name}</span><span>{payment.recorded_by_name}</span><strong className="payment-amount"><MoneyValue value={currency(payment.amount)} />{payment.voided_at && <small><RotateCcw /> Anulado</small>}</strong></div>)}</div>{!report.isPending && !report.data?.payments.length && <div className="empty-state"><Banknote /><h3>Sin movimientos</h3><p>No se registraron pagos en esta fecha.</p></div>}</section>
  </div>;
}
