...
export function ClientDetailPage({ technician = false, canViewBilling = false, canManageBilling = false }: { technician?: boolean; canViewBilling?: boolean; canManageBilling?: boolean }) {
  ...
  return <div className="page">
    ...
    {canViewBilling && !technician && <ClientAccountSection account={account.data} pending={account.isPending} clientName={client.data.name} canManagePayments={canManageBilling} onSelectService={(serviceId) => { setSelectedServiceTab("billing"); setSelectedService(serviceId); }} />}
    ...
    {selectedService && <ServicePanel serviceId={selectedService} initialTab={selectedServiceTab} onClose={() => setSelectedService(null)} allowFinance={canViewBilling && !technician} allowPaymentManagement={canManageBilling && !technician} />}
  </div>;
}
